...
function TestParse($directory)
{
	...
	foreach (glob("*.src") as $source) {
		//create $temp file
		$temp = "xkocur02.xml";
		$command = "php7.3 $parseDir < $source > $temp";
...
// echo "\n";
		...
		if ($return == $rc) {
			// echo "passed: test" . $testNumber . ":" . $directory . "/" . $source . "\n";
			// $test = TestPassed($directory, $source);
	$test = "passed: test" . $testNumber . ":" . $directory . "/" . $source;
			$htmlOutput .= "<tr style=\"background-color:green\"><td>". $test . "</td></tr>";
		$success++;
		}else 
		{
			// echo "failed: test". $testNumber . ":" . $directory . "/" . $source . "\n";
			// $test = TestFailed($directory, $source);
	$test = "failed: test" . $testNumber . ":" . $directory . "/" . $source;
...
function TestInt($directory)
{
	...
	foreach (glob("*.src") as $source) {
		...
		$input = $name[0] . ".in";
		$temp = "xkocur02";
		$command = "python3.6 $intDir > $temp $source $input"; 
